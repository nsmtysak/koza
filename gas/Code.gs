...
function industryPrimer(ctx) {
  ...
  return [
    ...
    '',
    '# 口座と信頼（この仕事の根幹）',
    '- 口座＝そのお客様がどのホステスのものか。北新地では永久指名制の店が多く、',
...
function objectivityRule() {
  return [
    '# 書き方のきまり（最重要）',
    'この記録は万一お客様の目に触れる可能性があります。次は書きません。',
    '- 感情や評価の語：機嫌、ケチ、面倒、感じが良い',
    '- 内面や体調の推測：疲労、疲れている、元気、緊張、体調が悪い、楽しそう、退屈そう',
    '- 推測を事実らしく見せる言い回し：〜そうだった、〜が見られた、〜な様子だった、〜と思われる',
    '',
    '書けるのは、その場で実際に見聞きした行動・発言だけです。',
    '  「機嫌が悪そうだった」→「いつもより口数が少なく、仕事の話が中心だった」',
    '  「ケチ」→「価格を確認してから注文された」',
    '',
    '**原文に観察された行動・発言が無い場合は、observation を空文字にします。**',
    '原文に無い行動を作り出してはいけません。',
    '  例：原文が「ちょっと疲れてる感じだった」だけの場合 → observation は空文字。',
    '      「いつもより口数が少なく」と書くのは捏造です。絶対にしないでください。',
    ''
  ].join('\n');
}
...
function inviteStyles() {
  return [
    ...
    '    例「◯◯が入りましたので、お知らせまで。」',
    '- deadline（期限をつける）',
    '    今でないと無くなる、という事実を添えます。作り話は絶対にしません。',
    '    例「頂いていたボトルが残り少なくなってまいりました。」',
    '- star（相手を主役にする）',
