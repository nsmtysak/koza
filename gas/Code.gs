...
function industryPrimer(ctx) {
  ...
  if (ctx.douhan_deadline) {
    extra.push('- 同伴は' + ctx.douhan_deadline + 'までに入店する必要があります。');
  }
  if (ctx.douhan_timeout_min) {
    extra.push('- 同伴のタイムアウトは' + ctx.douhan_timeout_min + '分です。超えると切り返しになり、伝票が2枚になります。');
  }
  if (ctx.douhan_deadline || ctx.douhan_timeout_min) {
    extra.push('  **ただし、これは店の数え方であって、お客様には関わりのない話です。**');
    extra.push('  お客様は「同伴」という言葉を、一度も聞いたことがないと思ってください。');
    extra.push('  時間に触れるときは、必ず**お客様にとっての都合**として書きます。');
    extra.push('    ✗「20時までにお入りいただければ同伴になります」');
    extra.push('    ✗「同伴が付きますので、7時にお願いできますか」');
    extra.push('    ○「7時頃にお食事を上がれましたら、ゆっくりしていただけます」');
    extra.push('    ○「早めにお会いできましたら、お店でも落ち着いてお話しできます」');
  }
...
function handleInvite(req) {
  ...
  var system = [
    ...
    '',
    '- **店の数え方を、文面に持ち込まないでください。**',
    '  同伴・切り返し・伝票・セット・ノルマ・指名——お客様は一度も聞いたことがない言葉です。',
    req.kind === 'douhan'
      ? '  同伴のお誘いでも、「同伴」という言葉は使いません。**お食事のお誘いとして書きます。**'
      : '',
    ctx.douhan_timeout_min && req.kind === 'douhan'
      ? '  時間の目安を添えるなら、お客様にとっての都合として書きます。' +
        '「7時頃に上がれましたら、ゆっくりしていただけます」のように。' : '',
    '',
    '# 文体',
